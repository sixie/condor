...
while ($i <= $nJobs) 
    ...
    set logFile=/tmp/Log_HerwigPP_${label}_${i}
...
Notification            = Error
Executable              = $script
Arguments               = $neventsPerJob $i $label $HiggsMass
Rank                    = Mips
Requirements            = (Arch == "INTEL") && (OpSys == "LINUX") && (Disk >= DiskUsage) && ((Memory * 1024) >= ImageSize) && (HasFileTransfer) 
GetEnv                  = True
Initialdir              = $workDir
Input                   = /dev/null
Output                  = $workDir/res/HerwigPP/$label/job_${i}.out
Error                   = $workDir/res/HerwigPP/$label/job_${i}.err
Log                     = $logFile
...
EOF
    ...
    rm submit.cmd
    echo $script $neventsPerJob $i $label $HiggsMass
